...
int main() {
  ...
  const std::string maximum_host(127, 'h');
  const std::string maximum_path = "/" + std::string(254, 'p');
  auto maximum_host_policy = policy();
  maximum_host_policy.allowed_firmware_host = maximum_host.c_str();
  const auto maximum_url = "http://" + maximum_host + maximum_path;
  assert(wio_ota_agent::parseManifest(
             manifest("wio-bg770a-v1.0", 129084, "2f4f", kValidSha256,
                      maximum_url.c_str())
                 .c_str(),
             maximum_host_policy, &parsed) ==
         wio_ota_agent::ManifestError::kNone);
  const auto oversized_host_url =
      "http://" + std::string(128, 'h') + "/firmware.bin";
  expectError(manifest("wio-bg770a-v1.0", 129084, "2f4f", kValidSha256,
                       oversized_host_url.c_str()),
              wio_ota_agent::ManifestError::kFirmwareUrlInvalid,
              maximum_host_policy);
  const auto oversized_path_url =
      "http://harvest-files.soracom.io/" + std::string(255, 'p');
  expectError(manifest("wio-bg770a-v1.0", 129084, "2f4f", kValidSha256,
                       oversized_path_url.c_str()),
              wio_ota_agent::ManifestError::kFirmwareUrlInvalid);
