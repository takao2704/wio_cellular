...
class ArduinoSupportTest(unittest.TestCase):
    ...
    def test_package_failure_preserves_existing_archive(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            def fail_after_first_member(package, *args, **kwargs):
                nonlocal writes
                writes += 1
                if writes == 2:
            ...
            with mock.patch.object(
                zipfile.ZipFile, "write", fail_after_first_member
            ):
                with self.assertRaises(OSError):
                    package_library(PROJECT, output)
            self.assertEqual(archive.read_bytes(), b"known-good-archive")
            self.assertEqual(list(output.glob(f".{LIBRARY}.zip.*")), [])
