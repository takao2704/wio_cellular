...
この単一ライブラリ構成では、HW v1.0の1台でPlatformIO生成アプリへのLTE OTAと、
そのアプリからArduino CLI生成アプリへのLTE OTAを確認しました。
署名付きmanifestとサービスFQDNを使い、全量取得・イメージ検証・再起動・新アプリ起動・更新なし判定まで確認しています。
さらに通常RESET後も適用バージョンを読み戻し、同じmanifestで再取得しないことを確認しました。
