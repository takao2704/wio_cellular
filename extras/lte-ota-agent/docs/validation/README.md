...
| [構成整理後のLTE OTA](public-layout-hardware-2026-08-31.md) | PlatformIO版、続いてArduino CLI版へ更新。検証・再起動・更新なし判定・RESET後の保存値読み戻し |
...
ライブラリのリリース番号ではありません。整理後の再実行結果は上の「構成整理後」の記録に分けています。
初期PoCのコードと当時のコマンドは
