...
class ProjectLayoutTest(unittest.TestCase):
    def test_example_and_hardware_environments_are_separate(self):
        example = config("examples/cellular-status-ota/platformio.ini")
        ...
        self.assertNotIn("HALT", (PROJECT / "examples/cellular-status-ota/platformio.ini").read_text())
        ...
        sketch = PROJECT / "examples/cellular-status-ota/CellularStatusOta/CellularStatusOta.ino"
    ...
    def test_user_guides_do_not_require_internal_test_names(self):
        ...
        guide = (PROJECT / "docs/ota-library-integration.md").read_text()
